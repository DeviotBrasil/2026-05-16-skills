#!/bin/bash
# sdd-cache-post.sh — Hook PostToolUse para WebFetch.
#
# Após WebFetch, armazena o corpo da resposta em .claude/sdd-cache/<sha>.json
# com ETag / Last-Modified atuais obtidos via HEAD para o hook pré revalidar
# no próximo fetch.
#
# Chave por URL. O prompt do chamador fica como metadado (não parte da chave)
# para um hit futuro mostrar qual pergunta gerou a leitura em cache.
# Entradas sem ETag ou Last-Modified não são cacheadas.
#
...
# Log de depuração: ativo com SDD_CACHE_DEBUG=1 ou sentinela em
# .claude/sdd-cache/.debug. Ligar/desligar com touch / rm.
dbg() {
...
# Forma de tool_response do WebFetch (Claude Code em 2026-04): objeto com
# chaves bytes, code, codeText, durationMs, result, url — conteúdo em .result.
# Outras chaves (.output / .text / .content / .body) são fallbacks defensivos
# se a forma mudar; jq retorna vazio se nada casar. Ramo string para integrações antigas.
...
# Deve coincidir com o hook pré: sha256(URL), primeiros 32 hex.
hash_key() {
...
# Capturar validadores da origem. Seguir redirects para bater com a URL
# que o agente realmente usou. Remover CR para o modo parágrafo do awk
# reconhecer separadores em cadeias de redirect.
HEAD_OUT=$(curl -sI -L --max-time 5 "$URL" 2>/dev/null | tr -d '\r' || true)

# Só cabeçalhos da resposta final (último parágrafo) para não pegar
# validadores de hops intermediários 301/302.
